...
do
	...
	then	
		...
		fileName=$(mysql cpadmin -u ${User} -p${Password} -h ${DB_HOST} -se "SELECT file_name FROM ssh WHERE deleted = 0 AND id = $keyId;")
		domainUser=$(mysql cpadmin -u ${User} -p${Password} -h ${DB_HOST} -se "SELECT UserName FROM domains, ssh WHERE domains.deleted = 0 AND ssh.deleted = 0 AND domains.id = ssh.domain_id AND ssh.id = $keyId;")
